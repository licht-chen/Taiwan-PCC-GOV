...
today = date.today()
headlist = ["項次", "機關名稱", "標案案號", "標案名稱", "公告次數", "公開閱覽期間", "功能選項",
            "公開閱覽公告網址"]
origin_href = "https://web.pcc.gov.tw/prkms/tpRead/common/readTpRead/advanced/returnToBasic?"
project_href = "https://web.pcc.gov.tw/tps/tp/common/TpReadSearch/showTpReadDetail?pkTpRead="
url = origin_href
### 特殊字元 replace
...
def pcc_gov_search_queryPublicReadData(orgname, startdate, enddate, search_type, mon_folder):
    # orgname = '中華郵政'
    # search_type = "公開徵求"
    # orgname = '中央銀行'
    # orgname = '金融監督管理委員會'
    ...
    #     if i == 0:
    #         url = origin_href + keys[i] + "=" + Payload[keys[i]] + "&"
    #     elif i < len(keys)-1:
    #         url = url + keys[i] + "=" + Payload[keys[i]] + "&"
    #     else:
    #         url = url + keys[i] + "=" + Payload[keys[i]]
    print("======", orgname, "======")
